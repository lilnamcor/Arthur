      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute left-1/2 -translate-x-1/2 top-full mt-2 w-48 bg-parchment rounded-lg shadow-lg border border-brown/20 py-2 z-50"
          >
            {socialLinks.map((link) => (
              <Link
                ...
                className="flex items-center px-3 py-2 text-sm text-slate hover:bg-burgundy/5 hover:text-burgundy transition-colors duration-200"
                onClick={() => setIsOpen(false)}
              >
                <span className="w-4 h-4 mr-3 flex items-center justify-center text-burgundy">
                  {link.icon}
                </span>
                {link.name}
              </Link>
