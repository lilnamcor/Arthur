    ...
    <main>
      ...
      <Section className={componentUtils.background.primary}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {themes.map((theme) => (
            <Card key={theme.title} className="group h-full flex flex-col">
              ...
              <p className={`${componentUtils.text.secondary} leading-relaxed mb-6 flex-grow`}>
                {theme.description}
              </p>
              <div className="prose prose-sm max-w-none">
                {theme.content.split('\n').filter(paragraph => paragraph.trim()).slice(0, 2).map((paragraph, i) => (
                  <p key={i} className={`mb-4 last:mb-0 ${componentUtils.text.secondary} leading-relaxed text-sm`}>
                    {paragraph.trim()}
                  </p>
                ))}
              </div>
